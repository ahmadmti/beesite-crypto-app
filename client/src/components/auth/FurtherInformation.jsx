...
export default function FurtherInformation() {
    ...
    const mail= ()=>{
        ...
        sendMail({
            from: `ru03244523131@gmail.com`,
            to: `usmanqadeer085@gmail.com`,
            sender: 'Pizza Reborn',
            subject: 'Email Verification',
            // html: `<p><a href="${process.env.APP_URL}/email-verify/${createdUser[0]}/${token}">Click on</a> this to verify Email</p>`,
        });
    }
    const handleSubmit = () => {
        setError('')
        ...
        if (activeStep === 9) {
            setError('')
            if (imageAsFile && proofImageAsFile) {
            handleNext()
            }
            else {
                setError("You have to upload documents")
            }
        }
        if (activeStep === 10) {
            setError('')
